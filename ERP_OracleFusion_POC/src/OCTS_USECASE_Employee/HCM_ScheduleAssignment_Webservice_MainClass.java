...
public class HCM_ScheduleAssignment_Webservice_MainClass extends ReporterBaseTest {

	 public static Boolean schAssignmentStatus=false;
	...
	@Parameters({ "testFile", "InterfaceFile" })
	 @Test
	public  void HCM_ScheduleAssignment(String inputfile_fp,String file_fp) throws Exception {
		try{
         ...
         */
		 ...
		 test = extent.createTest("Testing HCM Schedule Assignment Uploads");
		 System.out.println("after create test");
           ...
           try {
				     ...
				     Logger.logInfo(HCM_ScheduleAssignment_Webservice_MainClass.class, "File :" + inputfile_fp);
				  }
           catch (Exception e1) {
				     ...
				     Logger.logError(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 1: File is not in valid format.Please select correct file");
				     booleanstatus=false;
				     all.add(d);
				     e1.printStackTrace();
				     }
       ...
       //    XSSFRow row = ips1.getRow(0);
           ...
           Logger.logInfo(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 1: Data successfully imported");
       ...
       //    conn.setRequestProperty("Authorization", "Basic " + auth);
       	...
       	Logger.logInfo(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 2: Content converted to Base64");
           } catch (Exception e) {
   			// TODO Auto-generated catch block
        	   booleanstatus=false;
   			test.log(Status.FAIL, "Step 2: Content conversion to Base64 failed: " + e);
   			Logger.logError(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 2: Content conversion to Base64 failed: " + e);
   			e.printStackTrace();
		 ...
		 */
		   ...
		   ERP_uploadFileToUcm euftu = new ERP_uploadFileToUcm();
			...
			{
				booleanstatus=false;
				Logger.logError(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 3: uploadFileToUcm Failed.Please contact Functional team:"+resultuploadFile);
			...
			Logger.logInfo(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 3: uploadFileToUcm successful" );
			}
		}
					   ...
					   if(resultsubmitESSJobRequest_Loader.contains("Error"))
						{
							...
							Logger.logError(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 4: Load File to Interface unsuccessful: ");
							throw new Exception(resultsubmitESSJobRequest_Loader);
						}
					   else{
						   test.log(Status.PASS, "Step 4: Load File to Interface Successful" );
						   Logger.logInfo(HCM_ScheduleAssignment_Webservice_MainClass.class,"Step 4: Load File to Interface Successful");
			   ...
			   ERP_getESSJobStatus egejs = new ERP_getESSJobStatus();
					   ...
					   Thread.sleep(20000);
						...
						if (resultgetESSJobStatus_Loader.contains("Error")) {
							booleanstatus=false;
							Logger.logError(HCM_ScheduleAssignment_Webservice_MainClass.class,"Step 5: Load File to Interface Job resulted in Error, Check reason befor proceeding");
					   ...
					   Logger.logInfo(HCM_ScheduleAssignment_Webservice_MainClass.class,"Step 5: Load Import Job Status Successful" );
			}
		   ...
		   if(booleanstatus){
				   ...
				   ERP_submitESSJobRequest_RequestImport esejri = new ERP_submitESSJobRequest_RequestImport();
						   ...
						   if(resultsubmitESSJobRequest_ReqImport.contains("Error")){
							   ...
							   Logger.logError(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 7: Journal Import Job execution unsuccessful: " );
						   ...
						   else
						   {
							   ...
							   Logger.logInfo(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 7: HCM Import Job executed" );
					}
					}
					   ...
					   ERP_getESSJobStatus egejsi = new ERP_getESSJobStatus();
							   ...
							   Thread.sleep(20000);
								if (resultgetESSJobStatus_ReqImport.contains("Error")) {
									...
									Logger.logError(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 8: HCM Import Job resulted in Error, Check reason befor proceeding");
									throw new Exception(resultgetESSJobStatus_ReqImport);
								}

							   }
							   test.log(Status.PASS, "Step 8: HCM Import Job Status successful" );
							   Logger.logInfo(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 8: HCM Import Job Status successful" );
					}
						   ...
						   ERP_downloadESSJobExecutionDetails edejed = new ERP_downloadESSJobExecutionDetails();
								   ...
								   if (retunrstatus.contains("Error")) {
									   ...
									   Logger.logError(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 9: Download of HCM Import File unsuccessful");
										booleanstatus=false;
								   ...
								   if(f.exists()){
									...
									Logger.logInfo(HCM_ScheduleAssignment_Webservice_MainClass.class, "Step 9: Download of HCM Import File successful");
									   }
									   else{
