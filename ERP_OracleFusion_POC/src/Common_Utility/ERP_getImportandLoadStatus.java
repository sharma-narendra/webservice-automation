...
public class ERP_getImportandLoadStatus {

	String returnGetImportandLoadStatus;
	boolean booleanNode;

	public  String getImportLoadStatus(String reportId,String userName,String password) throws Exception {
		// TODO Auto-generated method stub
	/*	String reportId="212017";
		...
		String soapXml =  "<soap:Envelope xmlns:soap=\"http://www.w3.org/2003/05/soap-envelope\" xmlns:pub=\"http://xmlns.oracle.com/oxp/service/PublicReportService\">\r\n" +
	          ...
	          //  returnGetImportandLoadStatus=output;

	            returnGetImportandLoadStatus= reportStatus(output);
	           // FileUtils.writeStringToFile(new File("C:\\Automation_OCTS\\Data\\naren_Output.xml"), output);
				}catch (Exception e) {
					returnGetImportandLoadStatus="ERP_getImportandLoadStatus: Error occurred while sending SOAP Request to Server validating the Load Status";
		            System.err.println("\nError occurred while sending SOAP Request to Server!\nMake sure you have the correct endpoint URL and SOAPAction!\n");
		            e.printStackTrace();
		            return "Error"+e;
		        }
				return returnGetImportandLoadStatus;
	 ...
	 public String reportStatus(String output) {
		    ...
		    try {
		    FileUtils.writeStringToFile(new File("C:\\Automation_OCTS\\Data\\JobStatusResponse.xml"), output);
		    Thread.sleep(20000);
			File fXmlFile = new File("C:\\Automation_OCTS\\Data\\JobStatusResponse.xml");
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					IMPORTEDSTATUS=eElement.getElementsByTagName("IMPORTED_STATUS").item(0).getTextContent();
					TRANSFERSTATUS=eElement.getElementsByTagName("TRANSFER_STATUS").item(0).getTextContent();
					LOADEDSTATUS=eElement.getElementsByTagName("LOADED_STATUS").item(0).getTextContent();
			...
			if(booleanNode) {
				...
				if(IMPORTEDSTATUS.equals("SUCCESS") && TRANSFERSTATUS.equals("SUCCESS")  && LOADEDSTATUS.equals("SUCCESS")) {
					return "SUCCEEDED : Job imported, transferred and loaded successfully";
				}else {
					return "Error: Job not loaded successfully.Please check the error in the application";
				}
				...
				return "Error: Request not found in Soap response";
			...
			e.printStackTrace();
			returnGetImportandLoadStatus="ERP_getImportandLoadStatus: Error occurred while creating the xml from SOAP response validating the Load Status";
            System.err.println("\nError occurred while creating the xml from SOAP response validating the Load Status.. !\n");
            e.printStackTrace();
            return "Error"+e;
		    }
		  }
